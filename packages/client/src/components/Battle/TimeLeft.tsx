import { useTimestamp } from "../../mud/utils/useTimestamp";

const cx = 30;
const cy = 30;
const r = 15;


export const TimeLeft = (props: {startTimestamp: number, max_duration: number}) => {
            
  const {startTimestamp, max_duration} = props;

  const currentTimestamp = useTimestamp();

  const remainingSeconds = Math.max(0, startTimestamp + max_duration - currentTimestamp);
  const perimeter = 2 * Math.PI * r; // perimeter of the circle, assuming a radius of 50 pixels
  const progress = (max_duration - remainingSeconds) / max_duration;
  const remainingPerimeter = perimeter * (1 - progress);

  return (
    <svg width="100" height="100" style={{ position: "absolute", top: 10, left: 10, zIndex: 50}}>
      <circle cx={cx} cy={cy} r={r} stroke="#ccc" strokeWidth="2" fill="white" />
      <circle cx={cx} cy={cy} r={r} stroke="green" strokeWidth="3" strokeDasharray={`${remainingPerimeter} ${perimeter}`} transform={`rotate(-90 ${cx} ${cy})`} fill="none" />
      <text x={cx} y={cy} textAnchor="middle" alignmentBaseline="middle" fontSize="10">{Math.round(remainingSeconds)}</text>
    </svg>
  );
}